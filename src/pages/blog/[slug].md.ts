...
export const GET: APIRoute<{ post: CollectionEntry<'blog'> }> = ({ props }) => {
    const { post } = props;
    const { title, description, date, updated, slug, tags } = post.data;
    ...
    const updatedLine =
        updated !== undefined ? ` · Updated ${new Date(updated).toISOString().slice(0, 10)}` : '';
    const header = `# ${title}\n\n${summary}_${published}${updatedLine}_${tagLine}\n\nSource: https://dickwyn.com/blog/${slug}\n\n---\n\n`;
